...
3. 状態表示ラベルに接続用URL(`http://<このPCのIP>:<port>/docs`)と、その場でランダムに
   生成された API キー(`X-API-Key`)が表示されるので、これを利用したい他PCのユーザーに共有する。
   APIキーはサーバーを起動するたびに再生成される。
...
```
X-API-Key: <Start API Server 押下時に表示されたキー>
```
